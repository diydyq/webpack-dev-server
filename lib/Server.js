...
function Server(compiler, options) {
		...
		function writeDirectory(baseUrl, basePath) {
			...
			content.forEach(function(item) {
				...
				if(fs.statSync(p).isFile()) {
					...
					res.write(item);
					res.write('</a></li>');
					// Remove magic funcitonality.
					if(false && /\.js$/.test(item)) {
			...
			if (options.proxy) {
				...
				options.proxy.forEach(function (proxyOptions) {
					...
					app.all(proxyOptions.path, function (req, res, next) {
						// 重新赋值：使用新路由地址
						proxyOptions = (options.proxyLatest && options.proxyLatest[proxyOptions.path]) || proxyOptions;
						...
						if (bypassUrl) {
							...
							console.info('proxyOptions.path:', originalUrl, ' --> ', req.url);
	...
	// Watch proxy
	var webpackProxyFile = options.proxyfile;
	fs.watch(webpackProxyFile, function (eventname, filename) {

		// Monitor change events.
		if(eventname === 'change'){
			try{
				// 0. Clear cache
				delete require.cache[webpackProxyFile];
				// 1. Result
				options.proxyLatest = require(webpackProxyFile)({
					envMode: process.webpackEnv
				});
				// 2. Apply
				Object.keys(options.proxyLatest).forEach(function (path) {
					var proxyOptions;
					proxyOptions = options.proxyLatest[path];
					proxyOptions.path = path;
				});
				console.info('webpack: webpack.proxy.js changed!');
			}catch(e){
				console.error('webpack: webpack.proxy.js changed error: ', e.message);
			}
		}
